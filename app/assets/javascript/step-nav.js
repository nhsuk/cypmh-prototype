    ...
    this.start = function ($element) {
      ...
      function showPreviouslyOpenedSteps() {
        ...
        $.each($steps, function () {
          ...
          const stepView = new StepView($(this)); // show the step if it has been remembered or if it has the 'data-show' attribute
      ...
      function ensureOnlyOneActiveLink() {
        ...
        let lastClicked = loadFromSessionStorage(sessionStoreLink) || $element.find(`.${activeLinkClass}`).first().attr('data-position'); // it's possible for the saved link position value to not match any of the currently duplicate highlighted links
      ...
      function setShowHideAllText() {
        const shownSteps = $element.find('.step-is-shown').length; // Find out if the number of is-opens == total number of steps
...
const stepByStepNavigation = new GOVUK.Modules.AppStepNav();
